...
ARGO_BCRYPT="$(kubectl -n argocd exec deployment/argocd-server -- argocd account bcrypt --password password)"
kubectl -n argocd patch secret argocd-secret -p "{\"stringData\":{\"admin.password\":\"$ARGO_BCRYPT\",\"admin.passwordMtime\":\"$(date -u +%FT%TZ)\"}}"
